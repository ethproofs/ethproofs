...
export const rtpCohortSnapshots = pgTable(
  "rtp_cohort_snapshots",
  {
    id: bigint({ mode: "number" }).primaryKey().generatedByDefaultAsIdentity(),
    cluster_id: uuid("cluster_id")
      .notNull()
    ...
    sub_10s_proofs: integer("sub_10s_proofs").notNull(),
    over_10s_proofs: integer("over_10s_proofs").notNull(),
  ...
  (table) => [
    unique("rtp_cohort_snapshots_cluster_id_snapshot_week_uk").on(
      table.cluster_id,
      table.snapshot_week
    ),
