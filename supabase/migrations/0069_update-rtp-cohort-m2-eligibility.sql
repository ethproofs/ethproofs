...
DECLARE
  ...
  perf_score_threshold numeric := 70;
  liveness_score_threshold numeric := 95;
  m2_security_target_bits integer := 100;
  m2_proof_size_bytes bigint := 600 * 1024;
BEGIN
  ...
  SELECT
    ...
    (
      ...
      AND zsm.security_target_bits >= m2_security_target_bits
      AND zpm.size_bytes <= m2_proof_size_bytes
  ...
  LEFT JOIN zkvm_performance_metrics zpm ON zpm.zkvm_id = z.id
