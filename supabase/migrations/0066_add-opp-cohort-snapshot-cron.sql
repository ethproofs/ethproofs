...
DECLARE
  ...
  perf_score_threshold numeric := 80;
  liveness_score_threshold numeric := 98;
  ...
  SELECT
    ...
    (
      ...
      AND CASE WHEN tbc.cnt > 0
        THEN (COALESCE(cs.sub_threshold_proofs, 0)::numeric / tbc.cnt * 100) >= perf_score_threshold
        ELSE false
      END
      AND CASE WHEN tbc.cnt > 0
...
FROM cron.job
WHERE jobname = 'opp-cohort-snapshot-weekly';

SELECT cron.schedule(
  'opp-cohort-snapshot-weekly',
  '0 0 * * 1',
  'SELECT snapshot_opp_cohort();'
);
